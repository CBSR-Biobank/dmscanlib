...
// function call.
TW_IDENTITY ImageScanner::g_AppID = { 0, { 1, 0, TWLG_ENGLISH_USA, TWCY_USA,
...
const char * ImageScanner::TWAIN_DLL_FILENAME = "TWAIN_32.DLL";
...
ImageScanner::ImageScanner() :
   ...
   ua::Logger::Instance().subSysHeaderSet(2, "ImageScanner");
   ...
   if (g_hLib != NULL) {
      ...
      UA_ASSERTS(g_pDSM_Entry != 0,
                 "ImageScanner: Unable to fetch DSM_Entry address");
   }
}
...
bool ImageScanner::twainAvailable() {
   return (g_hLib != NULL);
}

unsigned ImageScanner::invokeTwain(TW_IDENTITY * srcId, unsigned long dg,
   ...
   if ((srcId == NULL) && (r != TWRC_SUCCESS) && (r != TWRC_CHECKSTATUS)) {
      UA_DOUT(2, 1, "ImageScanner::invokeTwain: unsuccessful call to twain");
   }
   return r;
}
/*
 ...
 */
bool ImageScanner::selectSourceAsDefault() {
 ...
 */
bool ImageScanner::scannerSourceInit(HWND & hwnd, TW_IDENTITY & srcID) {
...
void ImageScanner::scannerSourceDeinit(HWND & hwnd, TW_IDENTITY & srcID) {
...
void ImageScanner::setFloatToIntPair(const double f, short & whole,
                                     unsigned short & frac) {
 ...
 */
HANDLE ImageScanner::acquireImage(unsigned dpi, int brightness, int contrast,
...
HANDLE ImageScanner::acquireFlatbed(unsigned dpi, int brightness, int contrast) {
	TW_IDENTITY srcID;
	HWND hwnd;
...
DmtxImage* ImageScanner::acquireDmtxImage(unsigned dpi, int brightness,
                                          int contrast) {
   UA_ASSERT_NOT_NULL(g_hLib);
...
BOOL ImageScanner::setCapOneValue(TW_IDENTITY * srcId, unsigned Cap,
...
bool ImageScanner::getCapability(TW_IDENTITY * srcId, TW_CAPABILITY & twCap) {
   TW_UINT16 rc;
...
int ImageScanner::getScannerCapability() {
   TW_IDENTITY srcID;
   HWND hwnd;
...
int ImageScanner::getScannerCapabilityInternal(TW_IDENTITY & srcID) {
...
int ImageScanner::getResolutionCapability(TW_IDENTITY & srcID, TW_UINT16 cap) {
   pTW_RANGE pvalRange;
   bool result;
...
double ImageScanner::getPhysicalDimensions(TW_IDENTITY & srcID, TW_UINT16 cap) {
   bool result;
   TW_CAPABILITY twCap;
...
void ImageScanner::getCustomDsData(TW_IDENTITY * srcId) {
   TW_UINT16 rc;
...
inline double ImageScanner::uint32ToFloat(TW_UINT32 uint32) {
   TW_FIX32 fix32 = *((pTW_FIX32) (void *) (&uint32));
   return twfix32ToFloat(fix32);
}

inline double ImageScanner::twfix32ToFloat(TW_FIX32 fix32) {
 ...
 */
void ImageScanner::freeImage(HANDLE handle) {
