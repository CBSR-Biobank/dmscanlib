...
CSimpleOptA::SOption longOptions[] = { { OPT_ID_BRIGHTNESS, "--brightness",
		...
		SO_NONE }, { OPT_ID_DEBUG, "--debug", SO_REQ_SEP }, { OPT_ID_DEBUG_FILE,
		"--debugfile", SO_NONE }, { OPT_ID_DPI, "--dpi", SO_REQ_SEP }, {
		OPT_ID_GAP, "--gap", SO_REQ_SEP }, { OPT_ID_VERTICAL, "--vertical",
		SO_NONE }, { 'v', "-v", SO_NONE }, { 'h', "--help", SO_NONE }, { 'h',
		"--h", SO_NONE }, { 'i', "--input", SO_REQ_SEP }, { 'i', "-i",
		SO_REQ_SEP }, { 'p', "--plate", SO_REQ_SEP }, { 'p', "-p", SO_REQ_SEP },
		{ 'o', "--output", SO_REQ_SEP }, { 'o', "-o", SO_REQ_SEP }, { 's',
				"--scan", SO_NONE }, { 's', "-s", SO_NONE }, {
				OPT_ID_CAPABILITY, "--capability", SO_NONE }, { OPT_ID_SELECT,
				"--select", SO_NONE }, { OPT_ID_SQUARE_DEV, "--square-dev",
				SO_REQ_SEP }, { OPT_ID_THRESHOLD, "--threshold", SO_REQ_SEP }, {
				OPT_ID_LEFT, "--left", SO_REQ_SEP }, { OPT_ID_TOP, "--top",
		...
		if (args.LastError() == SO_SUCCESS) {
			...
			case OPT_ID_BRIGHTNESS:
				options.brightness = strtoul((const char *) args.OptionArg(),
						&end, 10);
				if (*end != 0) {
					cerr << "invalid value for brightness: " << args.OptionArg()
							<< endl;
					exit(1);
				}
				break;
			...
			case OPT_ID_DPI:
				options.dpi = strtoul((const char *) args.OptionArg(), &end,
						10);
				if (*end != 0) {
			...
			case OPT_ID_BOTTOM: {
				...
				if ((args.OptionId() == 'l')
					...
					options.left = num;
				} else if ((args.OptionId() == 't')
						|| (args.OptionId() == OPT_ID_TOP)) {
					options.top = num;
				} else if ((args.OptionId() == 'r')
						|| (args.OptionId() == OPT_ID_RIGHT)) {
					options.right = num;
				} else if ((args.OptionId() == 'b')
						|| (args.OptionId() == OPT_ID_BOTTOM)) {
					options.bottom = num;
