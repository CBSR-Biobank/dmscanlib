...
Dib::Dib(unsigned width, unsigned height, unsigned colorBits):fileHeader(NULL),
colorPalette(NULL)
{
...
void Dib::readFromFile(const char *filename)
{
	...
	r = fread(fileHeaderRaw, sizeof(unsigned char), sizeof(fileHeaderRaw),
		  fh);
	UA_ASSERT(r = sizeof(fileHeaderRaw));
	r = fread(infoHeaderRaw, sizeof(unsigned char), sizeof(infoHeaderRaw),
		  fh);
	...
	if (paletteSize > 0) {
		...
		r = fread(colorPalette, sizeof(unsigned char), paletteBytes,
			  fh);
...
bool Dib::writeToFile(const char *filename)
{
	...
	if (fileHeader != NULL) {
		...
		*(unsigned *)&fileHeaderRaw[2] = infoHeader->imageSize
		    + sizeof(fileHeaderRaw) + sizeof(infoHeaderRaw) +
		    paletteBytes;
	...
	*(unsigned *)&infoHeaderRaw[0x32 - 0xE] = infoHeader->numColorsImp;
	...
	UA_ASSERT(r == sizeof(fileHeaderRaw));
	r = fwrite(infoHeaderRaw, sizeof(unsigned char), sizeof(infoHeaderRaw),
		   fh);
	UA_ASSERT(r == sizeof(infoHeaderRaw));
	if (paletteSize > 0) {
		r = fwrite(colorPalette, sizeof(unsigned char), paletteBytes,
			   fh);
		UA_ASSERT(r == paletteBytes);
	}
...
inline unsigned char Dib::getPixelGrayscale(unsigned row, unsigned col)
{
	...
	if ((infoHeader->bitCount == 24) || (infoHeader->bitCount == 32)) {
		return static_cast <
		    unsigned char >(0.3333 * ptr[0] + 0.3333 * ptr[1]
				    + 0.3333 * ptr[2]);
	} else if (infoHeader->bitCount == 8) {
		return *ptr;
	}
...
void Dib::setPixel(unsigned x, unsigned y, RgbQuad & quad)
{
	...
	if (infoHeader->bitCount == 8) {
		*ptr =
		    static_cast <
		    unsigned char >(0.3333 * quad.rgbRed +
...
Dib *Dib::convertGrayscale(Dib & src)
{
	...
	// 24bpp -> 8bpp
	Dib & dest =
	    *(new Dib(src.infoHeader->width, src.infoHeader->height, 8));
	...
	for (unsigned row = 0; row < src.infoHeader->height; ++row) {
		srcPtr = srcRowPtr;
		destPtr = destRowPtr;
		for (unsigned col = 0; col < src.infoHeader->width;
		     ++col, ++destPtr) {
			*destPtr =
			    static_cast < unsigned char >(0.3333 * srcPtr[0]
							  + 0.3333 * srcPtr[1] +
							  0.3333 * srcPtr[2]);
...
void Dib::histEqualization(Dib & src)
{
	...
	unsigned char *srcRowPtr, *destRowPtr, *srcPixel, *destPixel,
	    pixelValue;
	...
	unsigned row, col, i;
	bool isRgb = (src.infoHeader->bitCount == 24)
	    || (src.infoHeader->bitCount == 32);
	...
	destRowPtr = pixels;
	for (row = 0; row < infoHeader->height;
	     ++row, srcRowPtr += src.rowBytes, destRowPtr += rowBytes) {
		srcPixel = srcRowPtr;
	...
	for (row = 0; row < infoHeader->height; ++row, destRowPtr += rowBytes) {
		...
		for (col = 0; col < infoHeader->width; ++col, destPixel
			...
			destPixel[0] =
			    static_cast <
			    unsigned char >(256 * sum[destPixel[0]]);
			if (isRgb) {
...
void Dib::convolveFast3x3(const float (&k)[9])
{
...

	{
		...
		for (unsigned row = 0; row < infoHeader->height; row++) {
			srcPtr = srcRowPtr;
			for (unsigned col = 0; col < infoHeader->width;
			     col++, srcPtr++, destPtr++) {
				*destPtr = *srcPtr;
			}
			srcRowPtr += rowBytes;
			...
			for (m = 0; m < kernelSizeY; ++m)	// kernel rows
			{
				...
				for (n = 0; n < kernelSizeX; ++n)	// kernel columns
				{
					...
					if (rowIndex >= 0
					    ...
					    && colIndex < dataSizeX)
						sum +=
						    pixels[rowBytes * rowIndex +
							   colIndex] *
						    kernel[kernelSizeX * mm +
							   nn];
				}
			}
			if (sum > 0)
...
void Dib::gaussianBlur(Dib & src)
{
	...
	for (i = 0; i < infoHeader->width - 1; ++i) {
		...
		for (j = 0; j < infoHeader->height - 1;
		     ++j, srcRowPtr += src.rowBytes, destRowPtr += rowBytes) {
			sumr = 0;
			sumg = 0;
			sumb = 0;
			for (k = 0; k < GAUSS_WIDTH; ++k) {
				...
				if ((x >= 0) && (x < infoHeader->width)) {
					...
					if ((infoHeader->bitCount == 24)
					    || (infoHeader->bitCount == 32)) {
						sumg +=
						    pixel[1] * GAUSS_FACTORS[k];
						sumb +=
						    pixel[2] * GAUSS_FACTORS[k];
					}
				}
			}
...
void Dib::blur(Dib & src)
{
	...
	for (k = 0; k < 2; k++) {
		...
		for (line = 0; line < nLines;
		     line++, pLineSrc += dLineSrc, pLineDest += dLineDest) {
			// loop through lines
			pPixelDest = pLineDest;

			for (pxl = 0, pPixelSrc = pLineSrc; pxl < d;
			     pxl++, pPixelDest += dPixelDest) {
				...
				if ((infoHeader->bitCount == 24)
				    || (infoHeader->bitCount == 32)) {
					pPixelDest[1] = pPixelDest[2] =
					    pPixelDest[0];
				}
			}

			for (pxl = d; pxl < nPixels - d;
			     pxl++, pPixelDest += dPixelDest) {
				// loop through pixels in main area
				pFactors = m_FilterVector;
				sum = 0;

				for (x = pxl - d, pPixelSrc =
				     &pLineSrc[x * dPixelSrc]; x <= pxl + d;
				     x++, pPixelSrc += dPixelSrc) {
					if ((infoHeader->bitCount == 24)
					    || (infoHeader->bitCount == 32)) {
						srcPixelValue =
			...
			for (pxl = nPixels - d; pxl < nPixels; pxl++, pPixelDest
				...
				for (x = xBegin, pPixelSrc =
				     &pLineSrc[x * dPixelSrc]; x < nPixels;
				     x++, pPixelSrc += dPixelSrc) {
					denom += *pFactors;
					if ((infoHeader->bitCount == 24)
					    || (infoHeader->bitCount == 32)) {
...
void Dib::unsharp(Dib & src)
{
	...
	for (Y = 0; Y <= height - 1; ++Y, srcRowPtr += rowBytes, destRowPtr
		...
		for (X = 0; X <= width - 1;
		     ++X, srcPixel += bytesPerPixel, destPixel +=
		     bytesPerPixel) {
			if ((infoHeader->bitCount == 24)
			    || (infoHeader->bitCount == 32)) {
				srcPixelValue =
