...
void PalletGrid::writeImageWithCells(std::string filename) {
	...
	for (unsigned i = 0, n = allCells.size(); i < n; ++i) {
		std::tr1::shared_ptr<const CvRect> rect = allCells[i]->getParentPos();
...
void PalletGrid::getCellImages() {
	...
	for (unsigned row = 0, rows = MAX_ROWS; row < rows; ++row) {
		...
		for (unsigned col = 0, cols = MAX_COLS; col < cols; ++col) {
			...
			std::tr1::shared_ptr<Dib> cellImage = filteredImage->crop(rect.x,
					rect.y, rect.x + rect.width, rect.y + rect.height);
			std::tr1::shared_ptr<PalletCell> cell(
					new PalletCell(cellImage, row, col, rect));
