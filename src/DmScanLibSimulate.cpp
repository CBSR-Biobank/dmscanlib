...
DmScanLibSimulate::DmScanLibSimulate(unsigned debugLevel, bool haveDebugFile)
: DmScanLib(debugLevel, haveDebugFile) {
}

int DmScanLibSimulate::isTwainAvailable() {
	return SC_TWAIN_UNAVAIL;
}

int DmScanLibSimulate::selectSourceAsDefault() {
	return SC_FAIL;
}

int DmScanLibSimulate::getScannerCapability() {
	return 0xFF; // supports WIA and DPI: 300,400,600
}

int DmScanLibSimulate::isValidDpi(int dpi) {
	return 1;
}

int DmScanLibSimulate::scanImage(unsigned verbose, unsigned dpi, int brightness,
		int contrast, double left, double top, double right, double bottom,
		const char *filename) {
	return SC_FAIL;
}

int DmScanLibSimulate::decodePlate(unsigned verbose, unsigned dpi,
		int brightness, int contrast, unsigned plateNum, double left,
		double top, double right, double bottom, double scanGap,
...
int DmScanLibSimulate::scanFlatbed(unsigned verbose, unsigned dpi,
		int brightness, int contrast, const char *filename) {
	return SC_FAIL;
}
